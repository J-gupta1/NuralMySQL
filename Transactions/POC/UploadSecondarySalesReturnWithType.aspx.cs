...
public partial class Transactions_POC_UploadSecondarySalesReturnWithType : PageBase
{
    ...
    private void InsertData(DataSet dsPrimarySales)
    {
        ...
        //for (int i = 0; i <= dsPrimarySales.Tables[0].Rows.Count - 1; i++)
        //{
        //    string strWhere = "RetailerCode='" + dsPrimarySales.Tables[0].Rows[i]["RetailerCode"].ToString().Trim() + "'and SalesMan <>'" + dsPrimarySales.Tables[0].Rows[i]["SalesMan"].ToString().Trim() + "'";
        //    if (dsPrimarySales.Tables[0].Rows[i]["RetailerCode"] != DBNull.Value)
        //    {
        //        DataRow[] dr = dsPrimarySales.Tables[0].Select(strWhere);
        //        if (dr.Length > 0)
        //        {
        //            counter = counter+1;
        //            if (dsPrimarySales.Tables[0].Rows[i]["Error"] == DBNull.Value && dsPrimarySales.Tables[0].Rows[i]["Error"] == string.Empty)
        //            {
        //                dsPrimarySales.Tables[0].Rows[i]["Error"] = "TD Can not have multiple salesman!";
        //            }
        //            else
        //                dsPrimarySales.Tables[0].Rows[i]["Error"] += ";TD Can not have multiple salesman!";
        //        }
    ...
    protected void DwnldReferenceCodeTemplate_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (SalesChannelData objSalesData = new SalesChannelData())
            {
                ...
                dsReferenceCode = objSalesData.GetAllTemplateDataWithType();
    ...
    protected void rdModelList_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("SecondarySalesReturnInterfaceWithType.aspx");
        }
    }
}
