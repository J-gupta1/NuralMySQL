...
public partial class Transactions_SalesChannel_UploadSecondarySales : PageBase
{
    ...
    private void InsertData(DataSet dsSecondarySales)
    {
        ...
        for (int i = 0; i <= dsSecondarySales.Tables[0].Rows.Count - 1; i++)
        {
            ...
            DateTime dtInvoiceDate = Convert.ToDateTime(dsSecondarySales.Tables[0].Rows[i]["SalesDate"]);
                ...
                {
                    counter = counter + 1;
                    if (dsSecondarySales.Tables[0].Rows[i]["Error"] != DBNull.Value && dsSecondarySales.Tables[0].Rows[i]["Error"].ToString() != string.Empty)
                ...
                //    {
                //        dsSecondarySales.Tables[0].Rows[i]["Error"] = Resources.Messages.ValidationSalesDays.ToString().Replace("Number", PageBase.NumberofBackDaysAllowed.ToString().Replace("-", ""));
                //    }
                //    else
                //        dsSecondarySales.Tables[0].Rows[i]["Error"] += ";" + Resources.Messages.ValidationSalesDays.ToString().Replace("Number", PageBase.NumberofBackDaysAllowed.ToString().Replace("-", ""));
                ...
                if (Convert.ToDateTime(SalesChannelOpeningStockDate).AddDays(-BackDaysAllowedBeforeOpening) >= Convert.ToDateTime(DateTime.Now.Date.AddDays(PageBase.NumberofBackDaysAllowed)))
                {
                    ...
                    if (Convert.ToDateTime(SalesChannelOpeningStockDate).AddDays(-PageBase.BackDaysAllowedBeforeOpening) > Convert.ToDateTime(dsSecondarySales.Tables[0].Rows[i]["SalesDate"]))
                    {
                        counter = counter + 1;
                        if (dsSecondarySales.Tables[0].Rows[i]["Error"] == DBNull.Value && dsSecondarySales.Tables[0].Rows[i]["Error"] == string.Empty)
                        {
                            dsSecondarySales.Tables[0].Rows[i]["Error"] = Resources.Messages.ValidationSalesDays.ToString().Replace("Number", (Convert.ToDateTime(DateTime.Now.Date.ToShortDateString()) - Convert.ToDateTime(Convert.ToDateTime(SalesChannelOpeningStockDate).AddDays(-BackDaysAllowedBeforeOpening).ToShortDateString())).TotalDays.ToString());
                        }
                        else
                            dsSecondarySales.Tables[0].Rows[i]["Error"] += ";" + Resources.Messages.ValidationSalesDays.ToString().Replace("Number", (Convert.ToDateTime(DateTime.Now.Date.ToShortDateString()) - Convert.ToDateTime(Convert.ToDateTime(SalesChannelOpeningStockDate).AddDays(-BackDaysAllowedBeforeOpening).ToShortDateString())).TotalDays.ToString());
                    }
    ...
    //protected void gvSecondarySales_PageIndexChanging(object sender, GridViewPageEventArgs e)
    //{
    //    try
    //    {
    //        gvSecondarySales.PageIndex = e.NewPageIndex;
    //        DataTable dt = new DataTable();
    //        gvSecondarySales.DataSource = (DataTable)ViewState["SecondarySales"];
    //        gvSecondarySales.DataBind();
    //    }
    //    catch (Exception ex)
    //    {
