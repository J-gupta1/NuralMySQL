 ...
 * Created On : 
 * Description : Uploading(Add/Edit) Orgnization Hierarchy Records from Excel Sheet.
 * ================================================================================================
 * Change Log: 11 June 2018
 * ------------- 
...
public partial class Masters_HO_Admin_ManageOrgnHierarchyUpload : PageBase
{
    ...
    protected void rdoSelectMode_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("ManageOrgnHierarchy.aspx");
        }
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (UploadCheck == 1)
            {
                ...
                isSuccess = UploadFile.uploadValidExcel(ref dsUpload, "OrgnHierarchy");
    ...
    protected void DwnldReferenceCode_Click(object sender, EventArgs e)
    {
        using (CommonData obj = new CommonData())
        {
            ...
            DataSet dsReferenceCode = obj.DownloadRefCodeExcel(8);
            ...
            if (dsReferenceCode != null && dsReferenceCode.Tables.Count > 0)
            {
                try
                {
                    ...
                    string FilenameToexport = "RefCodeOrgnHierarchy";
    ...
    private void InsertData(DataSet dsUpload)
    {
        ...
        for (int i = 0; i <= dtUpload.Rows.Count - 1; i++)
        {
            if (dtUpload != null && dtUpload.Rows.Count > 0)
            {
                ...
                string strColName1 = "LocationName = '" + dtUpload.Rows[i]["LocationName"].ToString().Trim() + "'";
                    ...
                    if (dr1.Length > 1)
                    {
                        ...
                        {
                            dtUpload.Rows[i]["Error"] = "LocationName supplied multiple times.";
                        }
                        else
                            dtUpload.Rows[i]["Error"] += ";LocationName supplied multiple times.";
                    }
                ...
                //Validate LocationCode
                    ...
                    string strColName2 = "LocationCode = '" + dtUpload.Rows[i]["LocationCode"].ToString().Trim() + "'";
                    ...
                    if (dr2.Length > 1)
                    {
                        ...
                        if (dtUpload.Rows[i]["Error"].ToString() == "" && dtUpload.Rows[i]["Error"].ToString() == string.Empty)
                        {
                            dtUpload.Rows[i]["Error"] = "LocationCode supplied multiple times.";
                        }
                        else
                            dtUpload.Rows[i]["Error"] += ";LocationCode supplied multiple times.";
                    }

                    ////Validate HierarchyLevelName
                    //    string strColName3 = "HierarchyLevelName = '" + dtUpload.Rows[i]["HierarchyLevelName"].ToString().Trim() + "'";
                    ...
                    //        if (dtUpload.Rows[i]["Error"].ToString() == "" && dtUpload.Rows[i]["Error"].ToString() == string.Empty)
                    //        {
                    //            dtUpload.Rows[i]["Error"] = "HierarchyLevelName supplied multiple times.";
                    //        }
                    //        else
                    //            dtUpload.Rows[i]["Error"] += ";HierarchyLevelName supplied multiple times.";
                    //    }
        ...
        try
        {
            ...
            if ((UpdateBCP(dt)) == true)
            {
                ...
                //{
                    ...
                    DataSet dsResult = obj.BulkUploadExcel("prcBulkOrgnHierarchyUpload");
    ...
    public bool UpdateBCP(DataTable dtUpload)
    {
        try
        {
            ...
            using (SqlBulkCopy bulkCopy = new SqlBulkCopy(PageBase.ConStr, SqlBulkCopyOptions.KeepIdentity))
            {
                ...
                bulkCopy.DestinationTableName = "BulkUploadOrgnHierarchy";
                bulkCopy.ColumnMappings.Add("LocationName", "LocationName");
                bulkCopy.ColumnMappings.Add("LocationCode", "LocationCode");
                bulkCopy.ColumnMappings.Add("HierarchyLevelName", "HierarchyLevelName");
                bulkCopy.ColumnMappings.Add("ParentHierarchyName", "ParentHierarchyName");
