 ...
 * Description : Uploading(Add/Edit) Model Records from Excel Sheet.
 * ================================================================================================
 * Change Log: 
 * ------------- 
...
public partial class Masters_Common_ManageModelUpload : PageBase
{
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (UploadCheck == 1)
            {
              ...
              //  UploadFile.UploadValidationType = EnumData.eUploadExcelValidationType.eOrgnHierarchy;

               isSuccess = UploadFile.uploadValidExcel(ref dsUpload, "ModelUpload");
               ...
               if (isSuccess == 1)
               {
                ...
                switch (isSuccess)
                {
                    ...
                    case 2:
                        ...
                        //#CC01 GridUpload.Columns[8].Visible = true;
    ...
    protected void DwnldReferenceCode_Click(object sender, EventArgs e)
    {
        using (CommonData obj = new CommonData())
        {
            ...
            DataSet dsReferenceCode = obj.DownloadRefCodeExcel(2);
    ...
    private void InsertData(DataSet dsUpload)
    {
        ...
        for (int i = 0; i <= dtUpload.Rows.Count - 1; i++)
        {
            if (dtUpload != null && dtUpload.Rows.Count > 0)
            {
                ...
                string strColName1 = "ModelName = '" + dtUpload.Rows[i]["ModelName"].ToString().Trim() + "'";
                ...
                string strColName2 = "ModelCode = '" + dtUpload.Rows[i]["ModelCode"].ToString().Trim() + "'";
                ...
                if (dr2.Length > 1)
                {
                    ...
                    if (dtUpload.Rows[i]["Error"].ToString() == "" && dtUpload.Rows[i]["Error"].ToString() == string.Empty)
                    {
                        dtUpload.Rows[i]["Error"] = "ModelCode " + Resources.Messages.UniqeMsg;
                    }
                    else
                        dtUpload.Rows[i]["Error"] += ";ModelCode " + Resources.Messages.UniqeMsg;
                }
                ...
                string strColName3 = "ProductName = '" + dtUpload.Rows[i]["ProductName"].ToString().Trim() + "'";
                ...
                if (dr2.Length > 1)
                {
                    ...
                    if (dtUpload.Rows[i]["Error"].ToString() == "" && dtUpload.Rows[i]["Error"].ToString() == string.Empty)
                    {
                        dtUpload.Rows[i]["Error"] = "ProductName " + Resources.Messages.UniqeMsg;
                    }
                    else
                        dtUpload.Rows[i]["Error"] += ";ProductName " + Resources.Messages.UniqeMsg;
                }

                //Validate SerialisedMode
                if (string.IsNullOrEmpty(dtUpload.Rows[i]["SerialisedMode"].ToString()))
                {
                    counter = counter + 1;
                    if (dtUpload.Rows[i]["Error"].ToString() == "" && dtUpload.Rows[i]["Error"].ToString() == string.Empty)
                    {
                        dtUpload.Rows[i]["Error"] = "SerialisedMode " + Resources.Messages.BlankMsg;
                    }
                    else
                        dtUpload.Rows[i]["Error"] += ";SerialisedMode " + Resources.Messages.BlankMsg;
                }

                if (!string.IsNullOrEmpty(dtUpload.Rows[i]["SerialisedMode"].ToString()) && dtUpload.Rows[i]["SerialisedMode"].ToString().Trim().ToLower() != "1")
                {
                    if (dtUpload.Rows[i]["SerialisedMode"].ToString().Trim().ToLower() != "2")
                    {
                        if (dtUpload.Rows[i]["SerialisedMode"].ToString().Trim().ToLower() != "3")
                        {
                            counter = counter + 1;
                            if (dtUpload.Rows[i]["Error"].ToString() == "" && dtUpload.Rows[i]["Error"].ToString() == string.Empty)
                            {
                                dtUpload.Rows[i]["Error"] = "Only 1 or 2 or 3 allowed in SerialisedMode.";
                            }
                            else
                                dtUpload.Rows[i]["Error"] += ";Only 1 or 2 or 3 allowed in SerialisedMode.";
                        }
                ...
                //Validate Model Type 
                if (string.IsNullOrEmpty(dtUpload.Rows[i]["ModelType"].ToString()))
                {
                    counter = counter + 1;
                    if (dtUpload.Rows[i]["Error"].ToString() == "" && dtUpload.Rows[i]["Error"].ToString() == string.Empty)
                    {
                        dtUpload.Rows[i]["Error"] = "ModelType " + Resources.Messages.BlankMsg;
                    }
                    else
                        dtUpload.Rows[i]["Error"] += ";ModelType " + Resources.Messages.BlankMsg;
                }

                if (!string.IsNullOrEmpty(dtUpload.Rows[i]["ModelType"].ToString()) && dtUpload.Rows[i]["ModelType"].ToString().Trim().ToLower() != "1")
                {
                    if (dtUpload.Rows[i]["ModelType"].ToString().Trim().ToLower() != "2")
                    {
                        counter = counter + 1;
                        if (dtUpload.Rows[i]["Error"].ToString() == "" && dtUpload.Rows[i]["Error"].ToString() == string.Empty)
                        {
                            dtUpload.Rows[i]["Error"] = "Only 1 or 2 allowed in ModelType.";
                        }
                        else
                            dtUpload.Rows[i]["Error"] += ";Only 1 or 2 allowed in ModelType.";
                    }
    ...
    //        //#CC01 GridUpload.Columns[8].Visible = false;
    //        ucMsg.Visible = false;
        ...
        try
        {
            ...
            if ((UpdateBCP(dt)) == true)
            {
                using (CommonData obj = new CommonData())
                {
                    ...
                    DataSet dsResult = obj.BulkUploadExcel("prcBulkModelUpload");
    ...
    public bool UpdateBCP(DataTable dtUpload)
    {
        try
        {
            ...
            using (SqlBulkCopy bulkCopy = new SqlBulkCopy(PageBase.ConStr, SqlBulkCopyOptions.KeepIdentity))
            {
                ...
                bulkCopy.DestinationTableName = "BulkUploadModel";
                ...
                bulkCopy.ColumnMappings.Add("ModelType", "ModelType");
                bulkCopy.ColumnMappings.Add("SerialisedMode", "SerialisedMode");
