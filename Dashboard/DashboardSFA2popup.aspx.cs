...
public partial class DashboardSFA2popup : System.Web.UI.Page
{
    ...
    private void GetUserTravelDistanceListData()
    {
        ...
        if (dsResult != null)
        {
            if (dsResult.Tables.Count > 0)
            {
                ...
                {
                    DataTable dt = dsResult.Tables[0];
                   lblYesterdayKm.Text = (Math.Round(Convert.ToDecimal(dt.Rows[0]["YesterdayKm"].ToString()))).ToString();
                   lblMTDKM.Text = (Math.Round(Convert.ToDecimal(dt.Rows[0]["MTDKM"].ToString()))).ToString();
                }
            }
        }
    ...
    private void GetUserPropectListData()
    {
        ...
        if (dsResult != null)
        {
            if (dsResult.Tables.Count > 0)
            {
                ...
                {
                    DataTable dt = dsResult.Tables[0];
                    lblYesterdayProspect.Text = (Math.Round(Convert.ToDecimal(dt.Rows[0]["YesterdayProspect"].ToString()))).ToString("#,##0");
                    lblMTDProspect.Text = (Math.Round(Convert.ToDecimal(dt.Rows[0]["MTDProspect"].ToString()))).ToString("#,##0");
                }
            }
        }
