 ...
 */
public partial class Login_Trans_Login : System.Web.UI.Page
{
                ...
                if (Validuser == true)
                {
                    ...
                    if (dtUser.Tables[0].Rows[0]["SalesChannelTypeID"].ToString() != "")
                    {
                        if (Convert.ToBoolean(dtUser.Tables[0].Rows[0]["IsOpeningStockEntered"]) == false)
                            ...
                            Response.Redirect("~/Transactions/POC/ManageOpeningStockBatch.aspx");   //Pankaj Dhingra(Due to BatchWise OpeningStock)
                        else
