        ...
        try
        {
            ...
            else
            {
                ...
                using (Authenticates ObjAuth = new Authenticates())
                {
                    ...
                    Validuser = ObjAuth.AuthenticateUser(clsRemoveHTMLTags.clsRemoveHTMLTags.fncRemoveHTMLTags(txtUsername.Text.Trim()), clsRemoveHTMLTags.clsRemoveHTMLTags.fncRemoveHTMLTags(txtPassword.Text.Trim()), ref dtUser, ref PsLockd,/*#CC02 Added*/txtCompanyName.Text.Trim(), 0);
                    /* #CC01 code Start */
                ...
                if (Validuser == true)
                {
                    ...
                    if (dtUser.Tables[0].Rows[0]["SalesChannelTypeID"].ToString() != "")
                    {
                        if (Convert.ToBoolean(dtUser.Tables[0].Rows[0]["IsOpeningStockEntered"]) == false)
                            ...
                            //Response.Redirect("~/Default.aspx", false);   //Pankaj dhingra*///Commented by Adnan
                           //Response.Redirect("~/MenuBindedPage.aspx", false);   
                           //Response.Redirect("~/Dashboard/DashboardSFA2.aspx", false);//Added by Adnan
                    }
                    else
                       //Response.Redirect("~/MenuBindedPage.aspx", false);//Commented by Adnan
                        Response.Redirect("~/Default.aspx", false);//Commented by Adnan
                        //Response.Redirect("~/Dashboard/DashboardSFA2.aspx", false);//Added by Adnan
                }
                else
                {
